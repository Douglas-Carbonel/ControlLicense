...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/permission-groups", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
  ...
  app.post("/api/permission-groups", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
  ...
  app.put("/api/permission-groups/:id", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      const group = await storage.updatePermissionGroup(id, req.body);
      res.json(group);
    } catch (error) {
  ...
  app.delete("/api/permission-groups/:id", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const id = parseInt(req.params.id);
      await storage.deletePermissionGroup(id);
      res.status(204).send();
    } catch (error) {
  ...
  app.get("/api/permission-groups/:id/menu-permissions", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const permissions = await storage.getMenuPermissions(groupId);
  ...
  app.post("/api/permission-groups/:id/menu-permissions", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const permission = await storage.setMenuPermission({
        ...req.body,
      ...
      res.status(500).json({ message: "Failed to set menu permission" });
    }
  });

  app.get("/api/permission-groups/:id/field-permissions", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const permissions = await storage.getFieldPermissions(groupId);
      res.json(permissions);
    } catch (error) {
      console.error("Error fetching field permissions:", error);
      res.status(500).json({ message: "Failed to fetch field permissions" });
    }
  });

  app.post("/api/permission-groups/:id/field-permissions", authenticateToken, requireAdmin, async (req: AuthRequest, res) => {
    try {
      const groupId = parseInt(req.params.id);
      const permission = await storage.setFieldPermission({
        ...req.body,
        permissionGroupId: groupId
      });
      res.json(permission);
      ...
      res.status(500).json({ message: "Failed to set field permission" });
    }
  });

  app.get("/api/users/:id/permissions", authenticateToken, async (req: AuthRequest, res) => {
    try {
      const userId = parseInt(req.params.id);
      const tableName = req.query.table as string || 'licenses';
      
      // Verificar se o usuário pode ver suas próprias permissões ou se é admin
      if (req.user!.id !== userId && req.user!.role !== 'admin') {
        return res.status(403).json({ message: "Access denied" });
      }

      const menuPermissions = await storage.getUserMenuPermissions(userId);
      const fieldPermissions = await storage.getUserFieldPermissions(userId, tableName);
